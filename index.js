...
app.post('/task',validateApiKey,async(req,res)=>{
...
app.put('/task',validateApiKey,async(req,res)=>{
    const title=req.body.title;
    const dueDate=req.body.dueDate;
    const jwt=req.cookies.jwt;
    const decoded=jwt.decode();
    ...
    if(!jwt){
        ...
        logger.error("User is not authorised!",Date.now())
    }
    try {
        mongoose.connect(process.env[url], {
            useNewUrlParser: true,
            ...
            sslValidate: true,
            sslCA: ca}, { useNewUrlParser: true, useUnifiedTopology: true });
            const result=  await mongoose.model('Task').findOne({post_id:post_id,userId:userId})
if(!result){
    res.status(404).send("Task not found!")
    logger.error("Task was not found!",Date.now())
}
if(result.userId!=userId){
    ...
    logger.error("User is not authorised!",Date.now())
            ...
            await mongoose.model('Task').updateOne({id:userId},{title:1,task_id:1})
    } catch (error) {
        res.status(500).send("Internal Server Error");
...
app.get('/task',validateApiKey,async(req,res)=>{
   ...
   if(!jwt){
    ...
    try {
        mongoose.connect(process.env[url], {
            ...
            sslValidate: true,
            sslCA: ca}, { useNewUrlParser: true, useUnifiedTopology: true });
          const result=  await mongoose.model('Task').find({userId:userId,post_id:post_id},{task_id:1,title:1,content:1})
if(!result){
    res.status(404).send("There are no tasks.")
    logger.error("User has no tasks.",Date.now())
}
    } catch (error) {
        res.status(500).send("Internal Server Error");
        logger.error(error,Date.now())
...
if(result.userId!=userId){
    ...
    logger.error("User is not authorised!",Date.now())
            ...
            await mongoose.model('Task').deleteOne({id:userId,post_id:post_id})
    } catch (error) {
        res.status(500).send("Internal Server Error");
    }finally{
        mongoose.client.close();
